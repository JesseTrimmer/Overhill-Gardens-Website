...
export default function Home() {
  return (
    <div className="fade-up" style={{ paddingTop: '72px' }}>
      ...
      {/* HERO */}
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', minHeight: 'calc(100vh - 72px)', background: 'var(--cream)' }}>
        <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: 'clamp(3rem,8vw,7rem) clamp(2rem,6vw,6rem)' }}>
          <span className="eyebrow">East Tennessee Native Plant Nursery</span>
          <h1 style={{
            fontFamily: "'Cormorant Garamond', serif", fontSize: 'clamp(2.8rem,5vw,5rem)',
            fontWeight: 300, lineHeight: 1.08, color: 'var(--charcoal)', marginBottom: '1.75rem',
          }}>
            Where your <em style={{ color: 'var(--moss)', fontStyle: 'italic' }}>natural garden</em> experience begins.
          </h1>
          <p style={{ fontSize: '1rem', fontWeight: 300, lineHeight: 1.85, color: 'var(--text-muted)', maxWidth: '44ch', marginBottom: '2.5rem' }}>
          ...
          <div style={{ display: 'flex', gap: '3rem', marginTop: '4rem', paddingTop: '2.5rem', borderTop: '1px solid var(--border)' }}>
            {[
              ...
              <div key={s.label}>
                <div style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: '2.4rem', fontWeight: 300, color: 'var(--moss)', lineHeight: 1 }}>{s.num}</div>
                <div style={{ fontSize: '0.7rem', letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--text-muted)', marginTop: '0.3rem' }}>{s.label}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ position: 'relative', overflow: 'hidden', minHeight: '500px' }}>
          <img src={gardenPhoto} alt="Native garden in bloom" style={{ width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'center' }} />
          <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(135deg, rgba(61,90,47,0.12), transparent 55%)' }} />
        </div>
      </div>

      {/* INTRO BAND */}
      <div style={{ background: 'var(--charcoal)', padding: '5rem clamp(2rem,8vw,8rem)', textAlign: 'center' }}>
        <h2 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 'clamp(1.6rem,3vw,2.5rem)', fontWeight: 300, color: 'var(--cream)', maxWidth: '70ch', margin: '0 auto', lineHeight: 1.5 }}>
        ...
        <p style={{ fontSize: '0.88rem', color: 'rgba(245,240,232,0.5)', marginTop: '1.5rem', letterSpacing: '0.05em' }}>
          We hope you'll join us in creating a legacy of sound stewardship!
        </p>
      </div>

      {/* TWO COLUMN CARDS */}
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2px', background: 'var(--parchment-dark)' }}>
        <Link
          to="/nursery"
          style={{ display: 'block', background: 'var(--white)', padding: '4rem clamp(2rem,5vw,5rem)', transition: 'background 0.25s', position: 'relative', overflow: 'hidden' }}
          onMouseEnter={e => e.currentTarget.style.background = 'var(--cream)'}
          onMouseLeave={e => e.currentTarget.style.background = 'var(--white)'}
        >
          <span className="eyebrow">Native Plant Nursery</span>
          <h2 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 'clamp(1.8rem,2.5vw,2.8rem)', fontWeight: 300, color: 'var(--charcoal)', marginBottom: '1.2rem', lineHeight: 1.15 }}>
          ...
          <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: '3px', background: 'var(--moss)' }} />
        </Link>

        <Link
          to="/landscaping"
          style={{ display: 'block', background: 'var(--moss)', padding: '4rem clamp(2rem,5vw,5rem)', transition: 'background 0.25s' }}
        ...
        >
          ...
          <h2 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 'clamp(1.8rem,2.5vw,2.8rem)', fontWeight: 300, color: 'var(--cream)', marginBottom: '1.2rem', lineHeight: 1.15 }}>
      ...
      {/* CONTACT BAND */}
      <div style={{ background: 'var(--parchment)', padding: '3rem 4vw', textAlign: 'center', borderTop: '1px solid var(--border)' }}>
